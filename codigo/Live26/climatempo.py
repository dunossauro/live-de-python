...
'''
para rodar esse arquivo digite: scrapy runspider .\climatempo.py
'''
...
class ClimatempoSpider(Spider):
    ...
    def start_requests(self):
        yield Request("https://www.climatempo.com.br/brasil", 
        callback=self.parse_start_page)

    def parse_start_page(self, response):
        yield FormRequest("https://www.climatempo.com.br/json/busca-por-nome",
                        formdata={"name": "Presidente Prudente"},
                        callback=self.parse_search)

    def parse_search(self, response):
        data = json.loads(response.body)
        city_id = data[0]["response"]["data"][0]["idcity"]
        yield Request("https://www.climatempo.com.br/previsao-do-tempo/15-dias/cidade/{}/".format(city_id),callback=self.parse)
