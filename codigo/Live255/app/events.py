...
class Memento:
    ...
    def new_state(self):
        return {'-CONTRASTE-': 0, '-BRILHO-': 0, '-COR-': 0, 'path': ''}
    ...
    def do(self, effect, factor, window):
        ...
        path, image_bytes, image = apply_effect(effect=effect, factor=factor)
        state['path'] = path
        state[effect] = factor
        state['object'] = image
...
def apply_effect(effect, factor):
    ...
    with NamedTemporaryFile(
        ...
        image = effects[effect](
            meme.states[meme.pos - 1]['path'], factor, tmp_path
        )
        return tmp_path.name, convert_image(tmp_path), image
